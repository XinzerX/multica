...
Multica 存储用户上传的附件（评论里的图片、文件等）。**配置了阿里云 OSS 时优先走 OSS**，否则走 S3，再否则回落本地磁盘。

### 阿里云 OSS

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `OSS_BUCKET` | 空 | Bucket 名。设了就启用 OSS 存储，并优先于 S3 |
| `OSS_REGION` | `cn-hangzhou` | OSS 区域，例如 `cn-hangzhou` |
| `OSS_ENDPOINT` | 空 | 可选的 OSS endpoint，例如 `https://oss-cn-hangzhou.aliyuncs.com` |
| `OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET` | 空 | 静态 OSS 凭证 |
| `OSS_SECURITY_TOKEN` | 空 | 可选 STS 临时凭证 token |
| `OSS_PUBLIC_BASE_URL` | 空 | 附件读取用的 CDN 或自定义域名。不设时返回 `https://<bucket>.oss-<region>.aliyuncs.com/<key>` |

### Release 发布

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `MULTICA_RELEASE_REPOSITORY` | 当前 CI 仓库 | Release 资产所在仓库，格式为 `owner/repo`；Web 下载页和 Desktop 打包都会读取它 |
...
**`OSS_BUCKET` 和 `S3_BUCKET` 都未设时**：所有上传回落到本地磁盘。
